package com.zdd.risk.utils.http;
...
public class HttpSendModel implements Serializable {
	...
	public String buildGetRequestUrl() {
		...
		if (params != null && params.size() != 0) {
			...
			/**
			 * 20140512 yfj modify requestUrl += "?" + appender;
			 */
			// 用于url不存在参数的情况
			if (url.indexOf("?") == -1) {
				requestUrl += "?" + appender;
	...
	public String buildPostRequestForm(String formName) {
		...
		buffer.append("<form id=\"").append(formName).append("\" name=\"").append(formName).append("\" action=\"")
				.append(url).append("\" accept-charset=\"").append(charSet).append("\" method=\""
						+ this.getMethod().value() + "\" onsubmit=\"if(isIE)document.charset='" + charSet + "'\" >\n");

		for (HttpFormParameter param : params) {
			if (param.isHidden()) {
				buffer.append("<input type=\"hidden\" name=\"").append(param.getName()).append("\" value=\"")
						.append(param.getValue()).append("\" />\n");
			} else {
				buffer.append("<input type=\"text\" name=\"").append(param.getName()).append("\" value=\"")
						.append(param.getValue()).append("\" />\n");
			}
		}
